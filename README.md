...
`update-all-clis` scans `~/.local/bin`, `~/.cargo/bin`, npm global bins, Homebrew Cellar, gem bins, and optionally directories on your `$PATH` — then runs the right update command for each. Nothing is hardcoded about *what* you have installed.
...
- **`path` (optional)** — With `--scan-path`, binaries found under `$PATH` directories get origin `path`. There is no default bulk update for `path`; add a `"path"` entry under `bulk` in [`config.local.json`](#configuration-merge-and-overrides) if you want a single command for all of them, or list tools under `known`.
...
./update_all_clis.sh --scan-path     # also scan dirs on $PATH (use with --rescan when first enabling)
...
1. **Discovery scan** — walks known tool directories and (if `--scan-path`) each directory on `$PATH`, and writes `~/.config/update-all-clis/cache.json`.
2. **Cache** — reused until it is older than **`CACHE_TTL_HOURS`** (default 24h), unless you pass **`--rescan`**. A normal run performs **at most one** full scan.
3. **`--no-scan`** — uses the existing cache when possible (see main script help for edge cases).
4. **Deduplication** — one bulk command per origin (e.g. one `npm update -g` for all npm globals). Known tools get their own command when listed in merged config.
5. **Execution** — sequential by default; **`--parallel=N`** runs multiple update steps concurrently (tracing is disabled for parallel runs).
